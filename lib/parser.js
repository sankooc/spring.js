...
exports.service = (filename, serviceName) => {
  LOG('start resolve', serviceName, 'at', filename);
  if (fs.existsSync(filename)) {
    ...
    for (let i = 0; i < statements.length; i += 1) {
      ...
      if (filterExport(state1)) {
        return { type: 'exports', name: serviceName, filename, params: getParams(state1) };
      }
...
exports.route = (filename, name) => {
  ...
  if (fs.existsSync(filename)) {
    const ast = parse(filename);
    const expresses = _.filter(_.get(ast, 'program.body'), (exp) => {
      return exp.type === 'ExpressionStatement' && _.get(exp, 'expression.left.object.name') === 'exports';
    })
    const map = {};
